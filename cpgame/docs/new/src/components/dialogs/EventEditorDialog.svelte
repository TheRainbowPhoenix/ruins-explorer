<script>
    ...
    function addPage() {
        localEvent.pages.push({ graphic: { tileId: 0 }, list: [] });
    ...
    function handleAddCommand(e) {
        ...
        if (indent) {
            if (code === 411) currentIndent = Math.max(0, currentIndent -1);
            if (code === 412) currentIndent = Math.max(0, currentIndent -1);
        }
...
<div class="dialog-overlay">
    <div class="dialog dialog-large">
        <div class="dialog-header">
            <h3>Edit Event: {localEvent.name}</h3>
            <button class="btn btn-secondary" on:click={close}>✕</button>
        </div>
        <div class="dialog-content-split">
            <!-- LEFT COLUMN -->
            <div class="dialog-column-left">
                <div class="form-group"><label class="form-label">Event Name</label><input type="text" class="form-input" bind:value={localEvent.name}></div>
                <div class="form-group"><label class="form-label">Page</label><div style="display: flex; gap: 8px;"><select class="form-select" bind:value={currentPageIndex} style="flex:1;">{#each localEvent.pages as _, i}<option value={i}>Page {i + 1}</option>{/each}</select><button class="btn btn-secondary" on:click={addPage}>+</button></div></div>
                <div class="form-group"><label class="form-label">Graphic</label><div class="graphic-picker">{#if tilesetLoaded}{#each Array(160) as _, i}<div class="graphic-item" class:selected={localEvent.pages[currentPageIndex].graphic.tileId === i} style="background-image: url('{tileset.src}'); background-position: -{(i % 16) * $tileSize * 2}px -{Math.floor(i / 16) * $tileSize * 2}px; background-size: {tileset.width * 2}px {tileset.height * 2}px;" on:click={() => localEvent.pages[currentPageIndex].graphic.tileId = i}></div>{/each}{/if}</div></div>
            </div>
            <!-- RIGHT COLUMN -->
            <div class="dialog-column-right">
                <div class="form-group"><label class="form-label">Commands</label><CommandList commands={localEvent.pages[currentPageIndex].list} on:edit={(e) => commandToEditIndex = e.detail}/></div>
...
{#if showAddCommand}
    <AddCommandDialog on:close={() => showAddCommand = false} on:add={handleAddCommand} />
{/if}
